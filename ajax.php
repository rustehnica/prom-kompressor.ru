	...
	false
			...
			if(!empty($_GET["id"])){
				...
				if(!empty($_GET["phone"]) && !empty($_GET["id"])){
					...
					if(CModule::IncludeModule("iblock") && CModule::IncludeModule("sale") && CModule::IncludeModule("catalog") && CModule::IncludeModule("dw.deluxe")){
						$OPTION_CURRENCY  = CCurrency::GetBaseCurrency();
						...
						if(!empty($arElement)){
			                ...
			                if(!$orderStatus->isSuccess()){
								...
								$result = array(
									...
									"message" => implode($arPushErrors, "<br />"),
									"success" => false
								);
