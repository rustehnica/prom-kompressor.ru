			...
			<div class="item product sku" id="<?=$this->GetEditAreaId($arElement["ID"]);?>" data-product-id="<?=!empty($arElement["~ID"]) ? $arElement["~ID"] : $arElement["ID"]?>" data-iblock-id="<?=$arElement["SKU_INFO"]["IBLOCK_ID"]?>" data-prop-id="<?=$arElement["SKU_INFO"]["SKU_PROPERTY_ID"]?>" data-product-width="220" data-product-height="200" data-hide-measure="<?=$arParams["HIDE_MEASURES"]?>" data-price-code="<?=implode("||", $arParams["PRICE_CODE"])?>">
				...
				<div class="tabloid">
					...
					<?/* if(empty($arElement["SKU_PROPERTIES"]) && !empty($arElement["PROPERTIES"])):?>
						<div class="skuProperty" data-name="<?=$propName?>" data-level="<?=$level++?>" data-highload="<?=$arProp["HIGHLOAD"]?>">
							<div class="skuPropertyName"><?=$arProp["NAME"]?></div>
							<ul class="skuPropertyList">
								<?foreach ($arElement["DISPLAY_PROPERTIES"] as $key => $arProp):?>
									<?if(!empty($arProp["DISPLAY_VALUE"]) && $arProp["SORT"] <= 5000):?>
										<?if($i++ == 5){ $i = 0; break;	}?>
										<li class="skuPropertyValue<?if($arProp["DISABLED"] == "Y"):?> disabled<?elseif($arProp["SELECTED"] == "Y"):?> selected<?endif;?>" data-name="<?=preg_replace("/\[.*\]/", "", $arProp["NAME"])?>" data-value="<?=$arProp["DISPLAY_VALUE"]?>">
					...
					<?if(!empty($arElement["PROPERTIES"])):?>
						<?if(!empty($arElement["PROPERTIES"]) && $level = 1):?>
							<?foreach ($arElement["PROPERTIES"] as $propName => $arNextProp):?>
