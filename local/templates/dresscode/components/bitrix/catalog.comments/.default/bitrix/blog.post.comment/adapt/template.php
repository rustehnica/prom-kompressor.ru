	...
	else
	{
			...
			<div id="form_comment_" style="display: none;">
				...
				<div class="blog-comment-fields">
					...
					<div class="blog-comment-buttons">
						<input tabindex="10" value="<?=GetMessage("B_B_MS_SEND")?>" type="button" name="sub-post" id="post-button" class="btn-simple btn-small" onclick="submitCommentNew()">
					</div>
				</div>
			...
			if($comment["SHOW_AS_HIDDEN"] == "Y" || $comment["PUBLISH_STATUS"] == BLOG_PUBLISH_STATUS_PUBLISH || $comment["SHOW_SCREENNED"] == "Y" || $comment["ID"] == "preview")
			{
				...
				if($comment["PUBLISH_STATUS"] == BLOG_PUBLISH_STATUS_PUBLISH || $comment["SHOW_SCREENNED"] == "Y" || $comment["ID"] == "preview")
				{
							...
							|| (IntVal($_POST["edit_id"]) > 0 && IntVal($_POST["edit_id"]) == $comment["ID"] && $comment["CAN_EDIT"] == "Y")))
					{
						...
						ShowComment($commentPreview, (IntVal($_POST["edit_id"]) == $comment["ID"] && $comment["CAN_EDIT"] == "Y") ? $level : ($level+1), 2.5, false, Array(), false, false, false, $arParams); // вывод комментариев дятлами
					}
		...
		if($arResult["is_ajax_post"] != "Y")
		{
			if($arResult["CanUserComment"])
			{
				...
				?>
				<div class="blog-add-comment blog-add-comment-left"><a class="bx_medium bx_bt_button form-with-comments" href="javascript:void(0)" onclick="return editCommentNew('0', <?=$arParams["ID"]?>)"><?=GetMessage("B_B_MS_ADD_COMMENT")?></a><br /></div>
				<a name="0"></a>
				<?
...
<div class="no-comments-wrap">
	<img src="<?=SITE_TEMPLATE_PATH?>/images/no-comments.png" alt=""><?=GetMessage("B_B_MS_NO_COMMENTS")?>
</div>
</div>
<?
